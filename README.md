...
Static landing at [www/index.html](www/index.html) — single self-contained file with CSS and JS inlined. POSTs to the same `/audit` endpoint as the HTTP wrapper. Rate-limited at the FastAPI layer via slowapi (3 audits per IP per day, keyed by `X-Forwarded-For`); higher-volume users get routed to the Apify Actor (paid path) by the in-page hint.
